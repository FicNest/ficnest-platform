...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/novels", async (req, res) => {
    try {
      ...
      const novels = await storage.getNovels(limit, offset);
      res.json(novels);
    } catch (error) {
  ...
  // Get draft chapters for a novel (author only)
  app.get("/api/novels/:novelId/drafts", isAuthor, async (req: Request, res: Response) => {
    try {
      ...
      if (novel.authorId !== req.user?.id) {
        return res.status(403).json({ message: "You can only view drafts for your own novels" });
      }
      ...
      const draftChapters = allChapters.filter(chapter => chapter.status === "draft");
      
      res.json(draftChapters);
    } catch (error) {
      console.error("Error fetching draft chapters:", error);
      res.status(500).json({ message: "Error fetching draft chapters" });
    }
  });
  ...
  app.get("/api/chapters/:id", async (req, res) => {
    try {
      ...
      // Get the chapter first to check its status and novelId
      const chapter = await storage.getChapter(id);
      if (!chapter) {
        return res.status(404).json({ message: "Chapter not found" });
      }

      // For draft chapters, only the author can view them
      if (chapter.status === "draft") {
        // Check if user is authenticated and get the novel
        if (!req.isAuthenticated()) {
        ...
        const isAuthor = req.user?.id === novel?.authorId;

        if (!isAuthor) {
          return res.status(403).json({
            message: "This chapter is not published yet",
            status: "draft"
          });
        }
      }

      // If published or if the user is the author of a draft, proceed to get the full chapter data
      // (The initial getChapter call above is sufficient for this logic, no need to re-fetch)

      // Only increment view count for published chapters
  ...
  app.post("/api/novels/:novelId/chapters", isAuthor, async (req: Request, res: Response) => {
    try {
      ...
      if (novel.authorId !== req.user?.id) {
        return res.status(403).json({ message: "You can only add chapters to your own novels" });
      }
  ...
  app.put("/api/chapters/:id", isAuthor, async (req: Request, res: Response) => {
    try {
      const id = Number(req.params.id);
      ...
      // Check if novel belongs to the author
      const novel = await storage.getNovel(existingChapter.novelId);
      if (novel?.authorId !== req.user?.id) {
        return res.status(403).json({ message: "You can only update chapters for your own novels" });
      }
  ...
  app.delete("/api/chapters/:id", isAuthor, async (req: Request, res: Response) => {
    try {
      const id = Number(req.params.id);
      ...
      const novel = await storage.getNovel(existingChapter.novelId);
      if (novel?.authorId !== req.user?.id) {
        return res.status(403).json({ message: "You can only delete chapters for your own novels" });
      }
  ...
  app.put("/api/chapters/:id/publish", isAuthor, async (req: Request, res: Response) => {
    try {
      const id = Number(req.params.id);
      ...
      // Check if novel belongs to the author (isAuthor middleware already does this, but double-checking)
      const novel = await storage.getNovel(existingChapter.novelId);
      if (novel?.authorId !== req.user?.id) {
        return res.status(403).json({ message: "You can only publish chapters for your own novels" });
      }

      // Update the chapter status to published
      const updatedChapter = await storage.updateChapter(id, { status: "published" });
    ...
    try {
      ...
      if (recentlyRead && recentlyRead.length > 0) {
        try {
          ...
          const chapter = await storage.getChapter(recentlyRead[0].chapterId);
  ...
  app.delete("/api/comments/:id", isAuthenticated, async (req: Request, res: Response) => {
    try {
      ...
      // Get the chapter to determine the novel
      const chapter = await storage.getChapter(comment.chapterId);
      if (!chapter) {
        return res.status(404).json({ message: "Chapter not found" });
      }
  ...
  app.get("/api/novels/search", async (req, res) => {
    try {
      ...
      // Filter novels that match either title or author
      const filteredNovels = [];
      
      for (const novel of allNovels) {
        // Check if novel title matches
        const titleMatches = novel.title.toLowerCase().includes(query.toLowerCase());
        
        // Check if author name matches (requires fetching author info)
        ...
        // Add novel if either title or author matches
