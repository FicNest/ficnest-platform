...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/novels/:novelId/chapters/:chapterNumber", async (req, res) => {
    try {
      ...
      // Check if chapter is published or if user is the author
      const isAuthor = req.user && req.user.id === chapter.authorId;
  ...
  app.post("/api/novels/:novelId/chapters", isAuthor, async (req: Request, res: Response) => {
    try {
      const novelId = Number(req.params.novelId);
      const { title, content, chapterNumber, status } = req.body;

      if (isNaN(novelId) || !title || !content || !chapterNumber || !status) {
