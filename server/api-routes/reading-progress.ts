  ...
  try {
    ...
    for (const progress of recentlyRead) {
      try {
        ...
        // Get chapter data
        const chapter = await storage.getChapter(progress.chapterId);
...
export async function getLatestReadingProgress(req: Request, res: Response) {
  try {
    ...
    try {
      ...
      // Get chapter data
      const chapter = await storage.getChapter(latestProgress.chapterId);
      ...
      console.log("Returning enhanced latest reading progress");
