...
export default function EditChapterPage() {
  ...
  useEffect(() => {
    const intervalId = setInterval(() => {
      ...
      if (values.title || values.content) {
        localStorage.setItem(`chapter-edit-${chapterId}`, JSON.stringify(values));
  ...
  const updateChapterMutation = useMutation({
    mutationFn: async (data: ChapterFormValues & { status: string }) => {
      const chapterData = {
        ...data,
        novelId, // Include the novelId
      };
      
      const res = await apiRequest("PUT", `/api/chapters/${chapterId}`, chapterData);
      return await res.json();
    },
    onSuccess: (chapter) => {
      // Clear saved draft
      localStorage.removeItem(`chapter-edit-${chapterId}`);
