...
function ChapterItem({ 
  ...
  chapter: Chapter;
  isAuthor: boolean;
  onDelete: (novelId: number, chapterNumber: number) => void;
}) {
  const [showDeleteDialog, setShowDeleteDialog] = useState(false);

  const handleDelete = () => {
    setShowDeleteDialog(false);
    onDelete(chapter.novelId, chapter.chapterNumber);
  };
...
export default function NovelDetailPage() {
  ...
  const deleteChapterMutation = useMutation({
    mutationFn: async ({ novelId, chapterNumber }: { novelId: number, chapterNumber: number }) => {
      await apiRequest("DELETE", `/api/novels/${novelId}/chapters/${chapterNumber}`);
    },
    onSuccess: () => {
  ...
  const handleDeleteChapter = (novelId: number, chapterNumber: number) => {
    deleteChapterMutation.mutate({ novelId, chapterNumber });
  };
