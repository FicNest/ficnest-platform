...
export default function SearchResultsPage() {
  ...
  const [key, setKey] = useState(Date.now()); // Use a key to force component refresh
  ...
  }, [window.location.search, location]); // Include both dependencies
  ...
  // Filter novels based on search query - including author names
  useEffect(() => {
    if (novels && searchQuery) {
      ...
      const filtered = novelsWithAuthor.filter(novel => {
        ...
        // Safe access to authorName with fallback
